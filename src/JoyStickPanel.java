...
public class JoyStickPanel extends Panel{
	...
	JoyStickPanel(String name, int x, int y, int width, int height, MainGround p, int cellSize, int resolution) {
		...
		ArrayList<String> joysticks = new ArrayList<String>();
		for(int i=0; i<p.controllIO.getNumberOfDevices(); i++) {
			joysticks.add(p.controllIO.getDevice(i).getName());
		}
	...
	public void drawPanel() {
		...
		if(connected) {
			innerPanel.setPosition(sliderX.getValue(), sliderY.getValue());
			throttlePanel.setSpeed(sliderZ.getValue());
		}
